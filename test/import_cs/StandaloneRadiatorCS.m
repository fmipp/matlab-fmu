...
% Instantiate the FMU.
time_out = 0;
visible = fmippim.fmi2False();
interactive = fmippim.fmi2False();

status = fmu.instantiate( 'standalone_radiator1', time_out, visible, interactive );
if status ~= fmippim.fmi2OK(); error( 'instantiation not successful' ); end

% Set value of parameter Tlow.
Tlow = 82.0;
status = fmu.setRealValue( 'Tlow', Tlow );
if status ~= fmippim.fmi2OK(); error( 'setRealValue not successful' ); end
...
% Initialize the FMU.
start_time = 0;
stop_time_defined = fmippim.fmi2True();
stop_time = 4 * 60 * 60;

status = fmu.initialize( start_time, stop_time_defined, stop_time );
if status ~= fmippim.fmi2OK(); error( 'initialzation not successful' ); end
...
while t < stop_time
	...
	new_step = fmippim.fmi2True;
    status = fmu.doStep( t, step_size, new_step )
	if status ~= fmippim.fmi2OK(); error( 'simulation step not successful' ); end
